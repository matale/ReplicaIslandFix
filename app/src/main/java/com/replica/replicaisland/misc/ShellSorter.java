...
public class ShellSorter<Type> extends Sorter<Type> {
     ...
     * Note that the running time can be tuned by adjusting the size of the
     * increment used to pass over the array each time.  Currently this function
     * uses Robert Cruse's suggestion of increment = increment / 3 + 1.
     */
    ...
    public void sort(Type[] array, int count, Comparator<Type> comparator) {
        ...
        // for calculating the next increment will keep generating 1 repeatedly.)
