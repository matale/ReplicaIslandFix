 ...
 * ArrayList.  It is designed to provide a contiguous array of fixed length
 * which can be accessed, sorted, and searched without requiring any runtime
 * allocation.  This implementation makes a distinction between the "capacity"
 * of an array (the maximum number of objects it can contain) and the "count"
 * of an array (the current number of objects inserted into the array).
 * Operations such as set() and remove() can only operate on objects that have
 * been explicitly add()-ed to the array; that is, indexes larger than
...
public class FixedSizeArray<T> extends AllocationGuard {
    ...
    /**
     * Swaps the element at the passed index with the element at the end of
     * the array.  When followed by removeLast(), this is useful for quickly
     ...
     * already been added to the array at that index for this command to complete.
     */
     ...
     * undefined state, so this function is potentially dangerous and should
     * be used in read-only cases.
     *
    ...
    /**
     * Searches the array for the specified object.  If the array was sorted
     ...
     * If the array is unsorted, a linear search is performed.
     * Note that if ignoreComparator is set to true, a linear search of object
     * references will be performed. Otherwise, the comparator set on this array
     * (if any) will be used to find the object.
     *
     * @param object The object to search for.
     * @return Index of the object in the array, or -1 if object is not found.
     */
    public int find(T object, boolean ignoreComparator) {
        ...
        if (sorted && !ignoreComparator && count > LINEAR_SEARCH_CUTOFF) {
            ...
            // object isn't found, but we just want a boolean.
            if (index < 0) {
                index = -1;
            }
        } else {
     ...
     * comparator has been specified with setComparator(), it will be used
     * for the sort; otherwise the object's natural ordering will be used.
     *
    ...
    /**
     * Returns maximum number of objects that can be inserted into this array.
     */
