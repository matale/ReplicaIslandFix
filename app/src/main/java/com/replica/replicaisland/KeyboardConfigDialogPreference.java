...
public class KeyboardConfigDialogPreference extends DialogPreference implements OnKeyListener {
    ...
    private View getConfigViewById(int id) {
        View config = null;
        if (id == R.id.key_left) config = mLeftBorder;
        if (id == R.id.key_right) config = mRightBorder;
        if (id == R.id.key_jump) config = mJumpBorder;
        if (id == R.id.key_attack) config = mAttackBorder;
        return config;
    }
