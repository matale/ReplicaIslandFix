...
import com.replica.replicaisland.constants.ButtonConstants;
...
public class InputGameInterface extends BaseObject {
    ...
    private final InputButton mJumpButton = new InputButton();
    ...
    public void update(float timeDelta, BaseObject parent) {
        ...
        if (mUseOnScreenControls) {
            ...
            mDirectionalPad.setMagnitude(filterOrientationForMovement(orientation.getX()),
            ...
            if (trackball.getLastPressedTime() > Math.max(leftPressedTime, rightPressedTime)) {
                // The trackball never goes "up", so force it to turn off if it wasn't
                // triggered in the last frame. What follows is a bunch of code to
                // filter trackball events into something like a dpad event.
                // The goals here are:
        ...
        final InputXY stompTouch = touch.findPointerInRegion(stompButtonRegionX,
        ...
        if (mUseClickButtonForAttack && clickButton.getPressed()) {
            ...
            // attack button here if it's not already down.  That makes it act like
            // the other buttons (down once then up).
        ...
        // but on the other hand, putting this code elsewhere causes dependencies
        // between exact HUD content and physics, which we sometimes wish to avoid.
