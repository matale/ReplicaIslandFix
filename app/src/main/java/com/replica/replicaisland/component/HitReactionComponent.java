...
import com.replica.replicaisland.game_objects.GameObjectFactory.GameObjectType;
...
/**
 * General-purpose component that responds to dynamic collision notifications.
 * This component may be configured to produce common responses to hit (taking
 * damage, being knocked back, etc) or it can be derived for entirely different
 * responses. This component must exist on an object for that object to respond
...
public class HitReactionComponent extends GameComponent {
    ...
    public void hitVictim(GameObject parent, GameObject victim, int hitType,
        ...
        if (hitAccepted) {
            ...
            if (mDealHitSound != null && (hitType == mDealHitSoundHitType ||
                    mDealHitSoundHitType == CollisionParameters.HitType.INVALID)) {
    ...
    public boolean receivedHit(GameObject parent, GameObject attacker, int hitType) {
        ...
        if (mGameEventHitType == hitType &&
            ...
            if (mLastGameEventTime < 0.0f || gameTime > mLastGameEventTime + EVENT_SEND_DELAY) {
                ...
                // special case.  If we're waiting for a hit type to spawn an event
                // and another event has just happened, eat this hit so we don't miss
    ...
    public void update(float timeDelta, BaseObject parent) {
        ...
        // giving all systems a chance to react.
