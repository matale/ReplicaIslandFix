...
/**
 * Implements rendering of a drawable object for a game object.  If a drawable
 * is set on this component it will be passed to the renderer and drawn on the
 * screen every frame. Drawable objects may be set to be "camera-relative"
 * (meaning their screen position is relative to the location of the camera
 * focus in the scene) or not (meaning their screen position is relative to
 * the origin at the lower-left corner of the display).
 */
public class RenderComponent extends GameComponent {
    private final Vector2 mPositionWorkspace;
    private final Vector2 mScreenLocation;
    private final Vector2 mDrawOffset;
    ...
    public void update(float timeDelta, BaseObject parent) {
        if (mDrawable != null) {
            ...
            if (system != null) {
                ...
                // render thread would allow us to have multiple views into the same
                // scene and things like that. But at the moment significant CPU is
                // being spent on sorting the list of objects to draw per frame, so
                // I'm going to go ahead and cull early.
                if (mDrawable.visibleAtPosition(mScreenLocation)) {
                    ...
                    // Normally the render system releases drawable objects back to the
                    // factory pool, but in this case we're short-circuiting the render
                    // system, so we need to release the object manually.
