...
import com.replica.replicaisland.systems.SoundSystem.Sound;

/**
 * Player Animation game object component.  Responsible for selecting an
 * animation to describe the player's current state.  Requires the object
 * to contain a SpriteComponent to play animations.
 */
public class AnimationComponent extends GameComponent {
    ...
    private ActionType mPreviousAction;
    ...
    public void update(float timeDelta, BaseObject parent) {
        if (mSprite != null) {
            ...
            boolean boosting = mPlayer != null && mPlayer.getRocketsOn();

            boolean visible = true;

            SoundSystem sound = sSystemRegistry.soundSystem;

            // It's usually not necessary to test to see if sound is enabled or not
            // (when it's disabled, play() is just a nop), but in this case I have a
            // stream that is maintained for the rocket sounds.  So it's simpler to
            // just avoid that code if sound is off.
            ...
            // object to play a sound when it is collected. The gems are a special
            // case, though, as we want to pick a different sound depending on how
