...
public class BackgroundCollisionComponent extends GameComponent {
    private final Vector2 mPreviousPosition;
    // Workspace vectors.  Allocated up front for speed.
    private final Vector2 mCurrentPosition;
    private final Vector2 mPreviousCenter;
    private final Vector2 mDelta;
    private final Vector2 mFilterDirection;
    private final Vector2 mHorizontalHitPoint;
    private final Vector2 mHorizontalHitNormal;
    private final Vector2 mVerticalHitPoint;
    private final Vector2 mVerticalHitNormal;
    private final Vector2 mRayStart;
    private final Vector2 mRayEnd;
    private final Vector2 mTestPointStart;
    private final Vector2 mTestPointEnd;
    private final Vector2 mMergedNormal;
    private int mWidth;
    private int mHeight;
    ...
    /**
     * Sets up the collision bounding box.  This box may be a different size
     * than the bounds of the sprite that this object controls.
     *
     ...
     * keep the collision box out of the collision world.
     * 1.	Cast a ray from the center point of the box at its position last frame
     * to the edge of the box at its current position.  If the ray intersects
     * anything, snap the box back to the point of intersection.
     * 2.	Perform Step 1 twice: once looking for surfaces opposing horizontal
     * movement and again for surfaces opposing vertical movement.  These two
     * ray tests approximate the movement of the box between the previous
     * frame and this one.
     * 3.	Since most collisions are collisions with the ground, more precision is
     * required for vertical intersections.  Perform another ray test, this
     * time from the top of the box's position (after snapping in Step 2) to
     * the bottom. Snap out of any vertical surfaces that the ray encounters.
     * This will ensure consistent snapping behavior on incline surfaces.
     * 4.	Add the normals of the surfaces that were hit up and normalize the
     * result to produce a direction describing the average slope of the
     * surfaces that the box is resting on. Physics will use this value as a
     * normal to resolve collisions with the background.
     */
    @Override
    public void update(float timeDelta, BaseObject parent) {
        ...
        if (mPreviousPosition.length2() != 0) {
            ...
            if (collision != null) {
                ...
                // The order in which we sweep the horizontal and vertical space can
                // affect the final result because we perform incremental snapping
                // mid-sweep.  So it is necessary to sweep in the primary direction
                // of movement first.
                if (Math.abs(mDelta.x) > Math.abs(mDelta.y)) {
                    horizontalHit = sweepHorizontal(mPreviousCenter, mCurrentPosition, mDelta, left,
                            right, centerOffsetY, mHorizontalHitPoint, mHorizontalHitNormal, parentObject);
                    verticalHit = sweepVertical(mPreviousCenter, mCurrentPosition, mDelta, bottom,
                            top, centerOffsetX, mVerticalHitPoint, mVerticalHitNormal, parentObject);
                } else {
                    verticalHit = sweepVertical(mPreviousCenter, mCurrentPosition, mDelta, bottom,
                            top, centerOffsetX, mVerticalHitPoint, mVerticalHitNormal, parentObject);
                    horizontalHit = sweepHorizontal(mPreviousCenter, mCurrentPosition, mDelta, left,
	...
	/*if (mCurrentPosition.y + bottom < 0.0f) {
	    ...
	    mVerticalHitNormal.normalize();
                ...
                // One more set of tests to make sure that we are aligned with the
                // surface. This time we will just check the inside of the bounding
                // box for intersections. The sweep tests above will keep us out of
                // collision in most cases, but this test will ensure that we are
                //aligned to incline surfaces correctly.
                ...
                if (mDelta.x != 0.0f && mDelta.y != 0.0f) {
                    ...
                    if (collision.castRay(mRayStart, mRayEnd, mFilterDirection, mVerticalHitPoint,
                        ...
                        // If we found a collision, use this surface as our vertical
                        // intersection for this frame, even if the sweep above also
                        // found something.
                        verticalHit = true;
                        // snap
                    ...
                    if (collision.castRay(mRayStart, mRayEnd, mFilterDirection,
                            mHorizontalHitPoint, mHorizontalHitNormal, parentObject)) {

                        // If we found a collision, use this surface as our horizontal
                        // intersection for this frame, even if the sweep above also
                        // found something.
                        horizontalHit = true;
                        // snap
                ...
                if (timeSystem != null) {
                    ...
                    // If we hit multiple surfaces, merge their normals together to
                    // produce an average direction of obstruction.
    ...
    protected boolean sweepHorizontal(Vector2 previousPosition, Vector2 currentPosition, Vector2 delta,
                                      int left, int right, float centerY, Vector2 hitPoint, Vector2 hitNormal, GameObject parentObject) {
        boolean hit = false;
        if (!Utils.close(delta.x, 0.0f)) {
            CollisionSystem collision = sSystemRegistry.collisionSystem;

            // Shoot a ray from the center of the previous frame's box to the edge
            // (left or right, depending on direction of movement) of current box.
            ...
            // direction, or push in the same direction as movement.
    ...
    protected boolean sweepVertical(Vector2 previousPosition, Vector2 currentPosition, Vector2 delta,
                                    int bottom, int top, float centerX, Vector2 hitPoint, Vector2 hitNormal, GameObject parentObject) {
        boolean hit = false;
        if (!Utils.close(delta.y, 0.0f)) {
            CollisionSystem collision = sSystemRegistry.collisionSystem;
            // Shoot a ray from the center of the previous frame's box to the edge
            // (top or bottom, depending on direction of movement) of current box.
