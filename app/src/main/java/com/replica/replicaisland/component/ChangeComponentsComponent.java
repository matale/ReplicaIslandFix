 ...
 * game object. The purpose of the ChangeComponentsComponent is to allow game
 * objects to have different "modes" defined by different combinations of
 * GameComponents. ChangeComponentsComponent manages the switching in and out
 * of those modes by activating and deactivating specific game components.
 */
public class ChangeComponentsComponent extends GameComponent {
    ...
    public void reset() {
        ...
        // GameComponents hanging out in the mComponentsToInsert list are not part
        // of the object hierarchy, so we need to manually release them.
        if (factory != null) {
            ...
            if (mActivated) {
                if (!mPingPong) {
                    ...
                    // mComponentsToInsert have already been inserted into the object
                    // and will be cleaned up with all the other of the object's
                    // components. In that case, mComponentsToRemove contains objects
                    // that need manual clean up.
