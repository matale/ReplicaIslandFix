...
public class DrawableBitmap extends DrawableObject {
    ...
    public void draw(float x, float y, float scaleX, float scaleY) {
        ...
        if (gl != null && texture != null) {
            ...
            if (!cull) {
                ...
                // This is necessary because we could be drawing the same texture with
                // different crop (say, flipped horizontally) on the same frame.
     ...
     * updated on the next call to draw(). Note that the image may be flipped
     * by providing a negative width or height.
     *
     * @param left
     * @param bottom
     * @param width
