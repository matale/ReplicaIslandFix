...
/**
 * An Axis-Aligned rectangular collision volume.  This code treats other
 * volumes as if they are also rectangles when calculating intersections.
 * Therefore certain types of intersections, such as sphere vs rectangle,
 * may not be absolutely precise (in the case of a sphere vs a rectangle,
 * for example, a new rectangle that fits the sphere is used to perform the
 * intersection test, so there is some potential for false-positives at the
 * corners). However, for our purposes absolute precision isn't necessary,
 * so this simple implementation is sufficient.
 */

public class AABoxCollisionVolume extends CollisionVolume {
    private final Vector2 mWidthHeight;
    ...
    /**
     * Calculates the intersection of this volume and another, and returns true if
     * the volumes intersect.  This test treats the other volume as an AABox.
     *
