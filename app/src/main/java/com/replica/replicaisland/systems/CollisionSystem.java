...
import com.replica.replicaisland.misc.HitPoint;
...
/**
 * Collision detection system.  Provides a ray-based interface for finding
 * surfaces in the collision world. This version is based on a collision
 * world of line segments, organized into an array of tiles.  The underlying
 * detection algorithm isn't relevant to calling code, however, so this class
 * may be extended to provide a completely different collision detection scheme.
 * <p>
 * This class also provides a system for runtime-generated collision segments.
 * These temporary segments are cleared each frame, and consequently must be
 * constantly re-submitted if they are intended to persist. Temporary segments
 * are useful for dynamic solid objects, such as moving platforms.
 * <p>
 * CollisionSystem.TileVisitor is an interface for traversing individual
 * collision tiles.  Ray casts can be used to run user code over the collision
 * world by passing different TileVisitor implementations to executeRay.
 * Provided is TileTestVisitor, a visitor that compares the segments of each
 * tile visited with the ray and searches for points of intersection.
 */
public class CollisionSystem extends BaseObject {
    private static final int MAX_TEMPORARY_SEGMENTS = 256;
    private final LineSegmentPool mSegmentPool;
    private final TileTestVisitor mTileSegmentTester;
    private final byte[] mWorkspaceBytes;     // Included here to avoid runtime allocation during file io.
    ...
    /*
     * Given a list of segments and a ray, this function performs an intersection
     * search and returns the closest intersecting segment, if any exists.
     */
    protected static boolean testSegmentAgainstList(FixedSizeArray<LineSegment> segments,
        ...
        for (int x = 0; x < count; x++) {
            ...
            // If a movement direction has been passed, filter out invalid surfaces
            // by ignoring those that do not oppose movement.  If no direction has
            // been passed, accept all surfaces.
            ...
            if (dot < 0.0f &&
                ...
                if (!foundHit || closestDistance > distance) {
                    ...
                    // Store the components on their own so we don't have to allocate
                    // a vector in this loop.
                    hitX = hitPoint.x;
                    hitY = hitPoint.y;
                }
            }
     ...
     * @param movementDirection If set, only segments with normals that oppose
     *                          this direction will be counted as valid intersections.  If null, all
     *                          intersecting segments will be considered valid.
     * @param hitPoint          Intersection point between ray and surface, if one found.
     * @param hitNormal         The normal of the intersecting surface if an intersection is found.
     * @param excludeObject     If set, dynamic surfaces from this object are ignored.
    ...
    public boolean castRay(Vector2 startPoint, Vector2 endPoint, Vector2 movementDirection,
        ...
        if (mTemporarySegments.getCount() > 0) {
            ...
            if (testSegmentAgainstList(mTemporarySegments, startPoint, endPoint,
                    tempHitPoint, tempHitNormal, movementDirection, excludeObject)) {
                if (hit) {
                    // Check to see whether this collision is closer to the one we
    ...
    /* Insert temporary surface into collision world. It'll persist for one frame. */
    ...
    public void update(float timeDelta, BaseObject parent) {
        ...
        // independent of frame execution order.  So each frame we queue up
        // inserted segments and then swap them into activity when this system is
        // updated.
     ...
     * executeRay() below, except that it is optimized for straight lines (either
     ...
     * @param hitPoint   Set to intersection coordinates if an intersection found.
     * @param hitNormal  Set to the normal of the intersecting surface if an
    ...
    public boolean loadCollisionTiles(InputStream stream) {
        ...
        // the size of the tileset, but at this point I don't actually know that
        // size, so I allocate a buffer that's probably large enough.
                ...
                if (byteStream.available() >= size) {
                    for (int x = 0; x < tileCount; x++) {
                        ...
                        for (int y = 0; y < segmentCount; y++) {
                            ...
                            // TODO: it might be wise to pool line segments.  I don't think
                            // that this data will be loaded very often though, so this is
                            // ok for now.
    ...
    protected class TileTestVisitor extends TileVisitor {
        ...
        // members to avoid runtime allocation.
        private final Vector2 mDelta;
        private final Vector2 mTileSpaceStart;
        private final Vector2 mTileSpaceEnd;
        private final Vector2 mTileSpaceOffset;
        private int mTileHeight;
        private int mTileWidth;
        ...
        /**
         * Sets the visitor up for a ray test.  Initializes the size of tiles and
         * the direction of movement by which intersections should be filtered.
         */
        ...
        public boolean visit(CollisionTile tile, Vector2 startPoint, Vector2 endPoint,
            ...
            boolean foundHit = testSegmentAgainstList(tile.segments, mTileSpaceStart,
                    mTileSpaceEnd, hitPoint, hitNormal, mDelta, null);
    ...
    protected class LineSegment extends AllocationGuard {
        ...
        private final Vector2 mEndPoint;
        ...
        /**
         * Checks to see if these lines intersect by projecting one onto the other
         * and then assuring that collision point is within range of each segment.
         */
