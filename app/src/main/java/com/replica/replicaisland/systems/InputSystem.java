...
/**
 * Manages input from a roller wheel and touch screen.  Reduces frequent
 * UI messages to an average direction over a short period of time.
 */
public class InputSystem extends BaseObject {
    private final InputTouchScreen mTouchScreen = new InputTouchScreen();
    private final InputXY mOrientationSensor = new InputXY();
    private final InputXY mTrackball = new InputXY();
    private final InputKeyboard mKeyboard = new InputKeyboard();
    private final float[] mOrientationInput = new float[3];
    private final float[] mOrientationOutput = new float[3];
    ...
    public void touchDown(int index, float x, float y) {
        ...
        TimeSystem time = sSystemRegistry.timeSystem;
        // Change origin of touch location from top-left to bottom-left to match
        // OpenGL space.
    ...
    public void setOrientation(float x, float y, float z) {
        // The order of orientation axes changes depending on rotation of screen.
        // Some devices call landscape "ROTATION_90" (e.g. phones), while others
        // call it "ROTATION_0" (e.g. tablets).  So we need to adjust the axes
        // from canonical space into screen space depending on the rotation of
        // the screen from whatever this device calls "default."
