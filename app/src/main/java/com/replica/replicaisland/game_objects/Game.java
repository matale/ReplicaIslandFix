...
import com.replica.replicaisland.MainActivity;
...
import com.replica.replicaisland.misc.MainLoop;
import com.replica.replicaisland.misc.MultiTouchFilter;
import com.replica.replicaisland.misc.SingleTouchFilter;
...
import com.replica.replicaisland.misc.TouchFilter;
...
public class Game extends AllocationGuard {
    private final ContextParameters mContextParameters;
     ...
     * Note that the game does not actually begin running after this function
     * is called (see start() below). Also note that textures are not loaded
     * from the resource pack by this function, as OpenGl isn't yet available.
     *
     * @param context
     */
    public void bootstrap(Context context, int viewWidth, int viewHeight, int gameWidth, int gameHeight, int difficulty) {
        if (!mBootstrapComplete) {
            ...
            // moves before the camera centers.
    ...
    protected synchronized void stopLevel() {
        ...
        //TODO: it's not strictly necessary to clear the static data here, but if
        // I don't do it then two things happen: first, the static data will refer
        // to junk Texture objects, and second, memory that may not be needed for
        // the next level will hang around.  One solution would be to break up the
        // texture library into static and non-static things, and then selectively
        // clear static game components based on their usefulness next level, but
        // this is way simpler.
