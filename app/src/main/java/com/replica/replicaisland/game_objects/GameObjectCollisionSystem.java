...
/**
 * A system for calculating collisions between moving game objects.  This
 * system accepts collision volumes from game objects each frame and performs
 * a series of tests to see which of them overlap.  Collisions are only
 * considered between offending "attack" volumes and receiving "vulnerability"
 * volumes.  This implementation works by using a sweep-and-prune algorithm:
 * objects to be considered are sorted in the x axis and then compared in one
 * dimension for overlaps.  A bounding volume that encompasses all attack and
 * vulnerability volumes is used for this test, and when an intersection is
 * found the actual offending and receiving volumes are compared.  If an
 * intersection is detected both objects receive notification via a
...
public class GameObjectCollisionSystem extends BaseObject {
    ...
    private static final CollisionVolume.FlipInfo sFlip = new CollisionVolume.FlipInfo();
    private static final CollisionVolume.FlipInfo sOtherFlip = new CollisionVolume.FlipInfo();
     ...
     * @param reactionComponent    A HitReactionComponent to notify when an
     *                             intersection is calculated. If null, the intersection will still occur
     *                             and no notification will be sent.
     * @param boundingVolume       A volume that describes the game object in space.
     *                             It should encompass all of the attack and vulnerability volumes.
     * @param attackVolumes        A list of volumes that can hit other game objects.
     *                             May be null.
     * @param vulnerabilityVolumes A list of volumes that can receive hits from
     *                             other game objects. May be null.
     */
    public void registerForCollisions(GameObject object,
                                      HitReactionComponent reactionComponent, CollisionVolume boundingVolume,
        ...
        for (int x = 0; x < count; x++) {
            ...
            for (int y = x + 1; y < count; y++) {
                ...
                if (otherPosition.x + other.boundingVolume.getMinXPosition(sOtherFlip) > maxX) {
                    // These objects can't possibly be colliding.  And since the list is
                    // sorted, there are no potentially colliding objects after this
                    // object either, so we're done!
                    break;
                } else {
                    ...
                    if (testRequired && record.boundingVolume.intersects(position, sFlip,
                        ...
                        final int hit = testAttackAgainstVulnerability(
                                record.attackVolumes, other.vulnerabilityVolumes,
                                position, otherPosition, sFlip, sOtherFlip);
                        ...
                        final int hit2 = testAttackAgainstVulnerability(
                                other.attackVolumes, record.vulnerabilityVolumes,
                                otherPosition, position, sOtherFlip, sFlip);
     ...
     * vulnerability volume, or HitType.INVALID if no intersections are found.
     */
    ...
    private final void drawDebugVolumes(CollisionVolumeRecord record) {
        ...
        if (mDrawDebugBoundingVolume) {
            ...
            sSystemRegistry.debugSystem.drawShape(
                    position.x + boundingVolume.getMinXPosition(sFlip), position.y
                            + boundingVolume.getMinYPosition(sFlip),
    ...
    /**
     * Comparator for game objects that considers the world position of object's
     * bounding volume and sorts objects from left to right on the x axis.
     */
    public final static class CollisionVolumeComparator implements Comparator<CollisionVolumeRecord> {
        private static final CollisionVolume.FlipInfo sCompareFlip = new CollisionVolume.FlipInfo();
