 ...
 * ignored, respectively) each frame based on the distance of that object to
 * the camera.  Objects may specify an "activation radius" to define an area
 * around themselves so that the position of camera can be used to determine
 * which objects should receive processing time and which should be ignored.
 * Objects that do not move should have an activation radius that defines a
 * sphere similar to the size of the screen; they only need processing when
 * they are visible.  Objects that move around will probably need larger
 * regions so that they can leave the visible area of the game world and not
...
public class GameObjectManager extends ObjectManager {
    ...
    private final float mMaxActivationRadius;
    private final FixedSizeArray<BaseObject> mInactiveObjects;
    private final FixedSizeArray<GameObject> mMarkedForDeathObjects;
    private final Vector2 mCameraFocus;
