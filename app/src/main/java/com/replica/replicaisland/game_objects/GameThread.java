 ...
 * the game graph, manages synchronization of input events, and handles the
 * draw queue swap with the rendering thread.
 */
            ...
            if (mGameRoot != null) {
                ...
                if (timeDelta > 12) {
                    ...
                    if (mProfileTime > PROFILE_REPORT_DELAY * 1000) {
                        ...
                        BaseObject.sSystemRegistry.hudSystem.setFPS((int) (1000 * mProfileFrames / mProfileTime));
                        mProfileTime = 0;
                        mProfileFrames = 0;
                    }
                }
                // If the game logic completed in less than 16ms, that means it's
                // running faster than 60fps, which is our target frame rate.  In
                // that case we should yield to the rendering thread, at least for
                // the remaining frame.
