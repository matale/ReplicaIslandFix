...
import com.replica.replicaisland.systems.RenderSystem.RenderElement;

import javax.microedition.khronos.egl.EGLConfig;
import javax.microedition.khronos.opengles.GL10;

/**
 * GameRenderer the top-level rendering interface for the game engine. It
 * called by GLSurfaceView and is responsible for submitting commands to
 * is OpenGL.  GameRenderer receives a queue of renderable objects from the
 * thread and uses that to draw the scene every frame.  If no queue is
 * available then no drawing is performed.  If the queue is not changed
...
public class GameRenderer implements GLSurfaceView.Renderer {
    ...
    private void hackBrokenDevices() {
        ...
        if (Build.PRODUCT.contains("morrison")) {
            ...
            // string is correct and the GL JNI glue is broken).
    ...
    /**
     * Draws the scene.
     * Note that the draw queue is locked for the duration of this function.
     */
    public void onDrawFrame(GL10 gl) {
        ...
        synchronized (this) {
            if (mDrawQueue != null && mDrawQueue.getObjects().getCount() > 0) {
                ...
                // queue that is empty, we'll leave the frame buffer alone.
                gl.glClear(GL10.GL_COLOR_BUFFER_BIT | GL10.GL_DEPTH_BUFFER_BIT);
            }
        }
    ...
    public synchronized void onPause() {
        ...
        // TODO: this is a hack.  Probably this renderer should
        // just use GLSurfaceView's non-continuious render mode.
        synchronized (mDrawLock) {
            mDrawQueueChanged = true;
            mDrawLock.notify();
        }
    ...
    /**
     * This function blocks while drawFrame() is in progress, and may
     * be used by other threads to determine when drawing is occurring.
     */
